...
pub(crate) async fn try_process(argv: Vec<&str>) -> Result<String, String> {
    ...
    match deactivate_user(&input.user_id, input.leave_rooms).await {
        ...
        Err(Errors::NotFrom) => {
            Err(format!("User {} is not from this homeserver", input.user_id))
        }
        Err(Errors::NotFound) => {
            Err(format!("User {} doesn't exist on this server", input.user_id))
        }
        Err(Errors::Error(e)) => Err(format!(
            "There was an error while trying to deactivate user {}: {e:?}",
            input.user_id
        )),
    }
}
